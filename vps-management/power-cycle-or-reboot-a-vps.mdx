...
description: "Control VPS power states from the EDBB portal—restart, stop, or start with a couple of clicks."
---

<Note>
  Prefer graceful reboots whenever possible: run <code>sudo shutdown -h now</code> or <code>sudo halt</code> inside Linux (or the equivalent command in Windows), then start the VPS from the portal. This keeps filesystems and services healthy before you resort to a forced power cycle.
</Note>
...
<Steps>
  <Step title="Open the VPS dashboard">
    Sign in to the <a href="https://my.edbb.com/clientarea.php" target="_blank">EDBB VPS Portal</a>, go to <strong>Services</strong>, and select the VPS you want to reboot.
  </Step>
...
![Restart server menu](/assets/vps-start-restart.png)
  </Step>
</Steps>
...
![Start server after shutdown](/assets/vps-stop-restart.png)
  </Step>
